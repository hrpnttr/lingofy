...
<div>
    ...
    <div class="bg-gradient-to-r from-blue-700 via-blue-800 to-indigo-900 text-white py-16 px-8 relative overflow-hidden shadow-sm">
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
            <div class="text-left">
                <h1 class="text-4xl font-extrabold tracking-tight">My Classroom</h1>
                <p class="mt-2 text-blue-100 text-lg font-medium">Continue your learning path and track your enrolled courses.</p>
            </div>
        </div>
    </div>
                ...
                <div class="lg:col-span-1 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-3">
                    ...
                    @foreach($enrolledContents as $index => $courseItem)
                        ...
                        >
                            ...
                            <svg class="w-4 h-4 shrink-0 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @foreach($enrolledContents as $index => $courseItem)
                        ...
                        >
                            ...
                            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm flex items-center justify-between">
                                <div>
                                    <span class="text-xs font-bold text-blue-600 uppercase tracking-wider">Active Course</span>
                                    <h2 class="text-2xl font-bold text-gray-900 mt-1">{{ $courseTitle }}</h2>
                                </div>
                                ...
                                <div class="space-y-6">
                                    @foreach($courseClasses as $classIndex => $classItem)
                                        <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                                            <!-- Class Header -->
                                            <div class="p-6 bg-slate-50/50 border-b border-gray-50 flex items-center justify-between">
                                                <div>
                                                ...
                                                <div class="space-y-4">
                                                    @if(is_array($classItem->materials) && count($classItem->materials) > 0)
                                                        @foreach($classItem->materials as $mIndex => $material)
                                                            @php
                                                                $materialId = 'material-' . $classItem->id . '-' . $mIndex;
                                                            @endphp
                                                            <div class="rounded-xl border border-gray-100 bg-white p-4 hover:border-gray-200 hover:shadow-sm transition-all duration-200">
                                                                <div class="flex items-center justify-between gap-4">
                                                                    <div class="flex items-center gap-3 min-w-0">
                                                                        <!-- Icon based on type -->
                                                                        <div class="shrink-0 rounded-xl p-2.5 
                                                                        ...
                                                                        <div class="min-w-0">
                                                                            ...
                                                                            <span class="text-xs text-gray-400 mt-0.5 block">
                                                                                @if(($material['type'] ?? '') === 'video') Watch video lecture
                                                                                @elseif(($material['type'] ?? '') === 'pdf') Read PDF module
                                                                                @else Solve homework test
                                                                                @endif
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div>
                                                                        @if(($material['type'] ?? '') === 'video' || ($material['type'] ?? '') === 'pdf')
                                                                            <button
                                                                                ...
                                                                                id="btn-{{ $materialId }}"
                                                                                class="shrink-0 rounded-xl bg-blue-600 px-5 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-blue-700 focus:outline-none cursor-pointer"
                                                                            >
                                                                                Start
                                                                            </button>
                                                                        @else
                                                                            <a
                                                                                href="{{ route('quizzes.started', ['id' => $classItem->id]) }}"
                                                                ...
                                                                <!-- Embedded Video (hidden by default) -->
                                                                @if(($material['type'] ?? '') === 'video' && isset($material['fileName']))
                                                                    <div id="media-{{ $materialId }}" class="hidden mt-4">
                                                                        <video controls width="100%" class="rounded-xl shadow-md border border-gray-200 bg-black">
                                                                            <source src="/videos/{{ $material['fileName'] }}" type="video/mp4" />
                                                                            Your browser does not support the video tag.
                                                                        </video>
                                                                    </div>
                                                                @endif

                                                                <!-- Embedded PDF (hidden by default) -->
                                                                @if(($material['type'] ?? '') === 'pdf' && isset($material['fileName']))
                                                                    <div id="media-{{ $materialId }}" class="hidden mt-4 h-[70vh] rounded-xl overflow-hidden border border-gray-200">
                                                                        <iframe
                                                        ...
                                                        @endforeach
                                                    @else
                                                        <div class="text-sm text-gray-400 italic py-4 text-center border border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                            ...
                            @else
                                <div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm text-center text-gray-500">
...
<script>
    function toggleMaterialMedia(mediaId) {
        ...
        if (targetMedia.classList.contains('hidden')) {
            ...
            targetBtn.classList.remove('bg-blue-600', 'hover:bg-blue-700');
    ...
    function switchCourseTab(courseId) {
        ...
        buttons.forEach(btn => {
            if (btn.id === 'tab-btn-' + courseId) {
                ...
                btn.classList.remove('bg-gray-50/50', 'hover:bg-gray-100', 'text-gray-700', 'border-transparent');
            } else {
